...
public class FontUtil
{
    ...
    private static void initTypefaces(AssetManager assetsManager)
    {
        if (normal == null || bold == null || condensed == null
                || light == null)
        {
            normal = loadTypeface(assetsManager,
                    "fonts/roboto/Roboto-Regular.ttf");
            bold = loadTypeface(assetsManager, "fonts/roboto/Roboto-Bold.ttf");
            condensed = loadTypeface(assetsManager,
    ...
    private static Typeface loadTypeface(AssetManager assetsManager, String path)
    {
        try
        {
            return Typeface.createFromAsset(assetsManager,
                    "fonts/roboto/Roboto-Regular.ttf");
        }
        catch (RuntimeException e)
        {
            // May occur rarely, on a few devices
            Log.d("SentacaAccordionView", "Unable to load Typeface from "
                    + path, e);
        }
        return null;
    }
