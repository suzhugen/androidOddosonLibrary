...
import com.oddoson.android.camera.FlashlightManager;
...
public class CameraScanActivity extends Activity implements OnClickListener {
	...
	private static int autoFocusTime = 1000;// 间隔1s
	private ImageScanner scanner;
	private Image barcode;
	private boolean barcodeScanned = false;//扫描成功
	private boolean previewing = true;  //预览中
	private boolean decodeing = true;  //解码中
	...
	void initScanAnim(){
		...
		TranslateAnimation animation=(TranslateAnimation) AnimationUtil.loadAnimation(this,R.anim.barcode_line_translate);
	...
	private class CommonHandler extends Handler{
		...
		public void handleMessage(Message msg) {
			...
			case CameraScanActivity.status_decode_success:
				...
				for (Symbol sym : syms) {
					barcodeScanned = true;
					Intent mIntent = new Intent(CameraScanActivity.this, QrCodePreviewActivity.class);
	...
	public void startScan() {
		...
		if (barcodeScanned) {
			...
			mPreview = new CameraPreview(this, mCamera, previewCallback, autoFocusCB);
	...
	PreviewCallback previewCallback = new PreviewCallback() {
		public void onPreviewFrame(byte[] data, Camera camera) {
			if (!decodeing) {
				...
				//这里可以控制解码范围
				barcode = new Image(size.width, size.height, "Y800");// "Y800"，"GRAY"
			...
			while (previewing)
			{
				try {
					if (barcode!=null&&!decodeing) {
						...
						if (result != 0) {
							commonHandler.sendEmptyMessage(status_decode_success);
						}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode==RESULT_OK&&requestCode==999) {
			String result=ScanDecodeManager.scanningImage(FileUtils.getUriPath(this,data.getData()));
			if (result==null) {
